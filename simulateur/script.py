...
#terrain.ajouter_rectangle((1600, 33), 200, 500 - int(33/2))
terrain.ajouter_ellipse(800, 100, 50, 900, 500)
...
while continuer == True:
    ...
    # a arranger avec le nouvel algo, la c juste pour le test
        ...
        if choix == "droite":
            ...
            while not terrain.signal(robot.capteur_g):
                pass
        elif choix == "gauche":
            robot.vd = v * 100
            robot.vg = - (robot.vd * (robot.l - robot.d)) / (2 * robot.l)
            while not terrain.signal(robot.capteur_d):
                pass
